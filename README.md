...
Experimenting with dolphin emulator to support shuffling between games and other adjacent features such as objective-based multi-game randomizers and lockouts.

## What is a "WiiMix"?

A WiiMix refers to a new way to play wii games, consisting of shuffling between games with various different objectives for each game. WiiMixes can be loaded and shared based on your own custom configurations.
...
1. `mkdir build`
2. `cd build`
3. `cmake ..      # Add -DCMAKE_BUILD_TYPE=Debug for debugging`
4. `make -j $(nproc)`
5. `sudo make install`
